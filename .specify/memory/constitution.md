...
### CI/CD Gate — обязательные локальные проверки перед пушем

Перед `git push` ДОЛЖНЫ быть пройдены ВСЕ проверки локально:
1. **Backend тесты**: `cd backend && .venv/bin/python -m pytest tests/ --ignore=tests/e2e -v`
2. **Backend линт**: `cd backend && .venv/bin/python -m ruff check .`
3. **Frontend тесты**: `cd frontend && npx vitest run`
4. **Frontend линт**: `cd frontend && npx eslint src/`
5. **Frontend тайпчек**: `cd frontend && npx tsc --noEmit`

**⛔ Пуш ЗАПРЕЩЁН, пока все проверки не зелёные.** После пуша и мержа — проверить CI в GitHub. Если CI упал — починить, запушить fix, дождаться green.
