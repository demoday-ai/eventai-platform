...
journey
    ...
    section После DD
      Оценивает проекты в Telegram: 3: Эксперт
...
| Приглашение | Входящее сообщение | "По вашим интересам (NLP, Agents) → Зал 3. Проекты: ..." | Интерес | Теги не точные → нерелевантная комната | US-007 |
...
| Оценка | Бот запрашивает (после DD) | По каждому проекту: оценка 1-5 + комментарий | Усталость (после целого дня) | Много проектов = много оценок | US-019 |
