...
| `reminders.py` | `/reminders` | List, send |
...
| `reminder_service.py` | ~851 | Напоминания за день/час, расписание отправки |
...
| `ReminderBatch` | `reminder_batches` | status (PREVIEW/CONFIRMED/IN_PROGRESS), counts |
| `ReminderNotification` | `reminder_notifications` | batch_id, telegram_user_id, status |
