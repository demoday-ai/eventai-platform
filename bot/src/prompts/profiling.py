...
ROLE_CONTEXTS: dict[tuple[str, str | None], str] = {
    ("guest", "student"): (
        "Стиль: дружелюбный, на <<вы>> (как все сообщения бота), по-русски.\n"
    ...
    ("guest", "applicant"): (
        "Стиль: мотивирующий, на <<вы>> (как все сообщения бота), по-русски.\n"
...
def get_profile_agent_system(
    ...
    return (
        ...
        "Тон нейтрально-деловой. БЕЗ восклицаний и оценок ('Отлично', 'Хороший выбор', "
        "'Широкий набор', 'Отличный набор'). Сразу по делу.\n\n"
        ...
        "2. Если в первом сообщении УЖЕ есть интересы И цель/задача (или роль) - можешь "
        "СРАЗУ вернуть action=profile, не переспрашивая. Уточняющий вопрос (action=reply) "
        "задавай ТОЛЬКО когда ввод расплывчатый или цели не видно - и тогда лишь ОДИН.\n"
        ...
        '  "message": "Уточните: вам ближе чат-боты и RAG,\n'
        ...
        'Assistant: {{"action": "reply",\n'
        '  "message": "Какая основная задача: классификация дефектов по фото\n'
        ...
        'Пример 4 (насыщенный первый ввод - фиксируем СРАЗУ, без переспроса):\n'
        'User: "интересуюсь NLP, RAG и агентами, цель - внедрять в прод"\n'
        'Assistant: {{"action": "profile",\n'
        '  "interests": ["NLP", "RAG", "Agents", "LLM"],\n'
        '  "goals": ["Найти решения, готовые к внедрению в прод"],\n'
        '  "summary": "Интересуется NLP, RAG и агентами, ищет проекты, готовые к внедрению."}}\n'
        "===КОНЕЦ ПРИМЕРОВ==="
    )
