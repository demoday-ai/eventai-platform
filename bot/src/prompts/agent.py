...
def build_agent_system_prompt(
    ...
    return (
        ...
        "- update_program - быстро поправить программу без пересборки: убрать проект (remove) или исключить тему (exclude)\n"
        ...
        "- Изменить подборку: убрать проект или тему -> update_program(remove=[номера], exclude=[темы]); "
        "сменить или расширить интересы ('больше про X', 'ещё интересует Y') -> rebuild_program(note). "
        "НЕ переписывай порядок программы текстом и НЕ советуй /rebuild. "
        "Свободно советовать и предупреждать о конфликтах можно, но СОСТАВ программы меняют ТОЛЬКО эти инструменты\n"
        ...
        "update_program, rebuild_program) "
